...
createhost() {
    JsData=`jo jsonrpc=2.0 method=host.create params=$(jo host=$HOSTNAME_ag interfaces=$(jo type=1 main=1 useip=1 ip=$ZbAgentIp dns="" port=$ZbPort) groups=$(jo groupid=10001)) templates=$(jo templateid=1001 templateid=$TEMPL_ID) auth=$AUTH_TOKEN id=0`
