#!/bin/bash
DBpass='zabbix'
IP_srv="192.168.56.121"
HOSTNAME_ag="zabbix-ag12"

function install_srv(){
DBpass='zabbix'
sudo rpm -Uvh https://repo.zabbix.com/zabbix/4.4/rhel/7/x86_64/zabbix-release-4.4-1.el7.noarch.rpm
sudo  yum clean all 
sudo yum -y install zabbix-server-pgsql zabbix-web-pgsql zabbix-apache-conf zabbix-agent zabbix-get
...
sudo -i -p -u postgres psql -c "ALTER USER zabbix WITH PASSWORD $DBpass";
...
#DBPassword=$DBpass
#> 
sudo -p sed -i.bak '/# DBPassword=.*/DBPassword=$DBpass/' /etc/zabbix/zabbix_server.conf
